...
4. Implement one C++ function that reproduces the behavior.
   - If the range does not end in an `RTS`, `JMP`, or unconditional branch, it falls through into the next label (`end_label`). Model that fall-through in C++ by calling the following function at that point.
