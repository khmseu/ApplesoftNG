...
**Current Progress**: `STOP()`, `ENDX()`, `CONT()`, `GOSUB()`, `GO_TO_LINE()`, `GOTO()`, `POP()`, `RETURN()`, `GOEND()`, `COLON_()`, `EXECUTE_STATEMENT()`, `EXECUTE_STATEMENT_1()`, `TRACE_()`, `STEP()`, `NEWSTT()`, `IF()`, `REM()`, `IF_TRUE()`, `ONGOTO()`, `CONTROL_C_TYPED()`, and `RESTART()` now live in `asm_port_control_flow.cpp`.
...
**Current Progress**: `SAVE()`, `LOAD()`, `VARTIO()`, `PROGIO()`, `ADDON()`, `DATAN()`, `DATA()`, `LET()`, `LET2()`, `PUTSTR()`, `DEL()`, `LIST()`, `PARSE_INPUT_LINE()`, numbered-line helpers, line-search helpers (`FNDLIN()`/`FL1()`), and the `FIX_LINKS` support cluster now live in `asm_port_statements.cpp`; parser-side helpers `IsStatementEndOfParsedInput()` and `CONINT()` now live in `asm_port_parser.cpp`; line/error-number output helpers (`PrintDecimalUnsigned()`/`LINPRT()`/`INPRT()`/`STROUT(std::string_view)`) now live in `asm_port_print.cpp`; stack initialization helper `STKINI()` now lives in `asm_port_stack.cpp`; control-break poll helper `ISCNTC()` now lives in `asm_port_control_flow.cpp`; obsolete legacy helpers (including unused `GETCHR` and `GOERR`) were removed from `asm_port_error.cpp`, leaving mostly pointer-fix/runtime plumbing there.
...
- ⏳ Extract parsing/tokenization functions (in progress: `SYNCHR`/`CHKNUM`/`CHKSTR`/`CHKCOM`/`CHKCLS`/`CHKOPN` + `LINGET`/`GTBYTC`/`GETBYT`/`GTNUM`/`COMBYTE`/`GETADR` + `GENERIC_CHRGET`/`CHRGET`/`CHRGOT` + `CurrentStatementChar`/`RESTORE`/`SETDA` moved to `asm_port_parser.cpp`)
