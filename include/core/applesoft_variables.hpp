...
public:
    ...
    static constexpr std::uint8_t ZP_AS_LENGTH = 0x91; // Dual-use byte in ROM: AS_LENGTH shares storage with AS_JMPADRS low byte.
    ...
    static constexpr std::uint8_t ZP_AS_FAC_MANTISSA = 0x9e; // Start of 4-byte mantissa (9e, 9f, a0, a1)
    ...
    static constexpr std::uint8_t ZP_AS_ARG_MANTISSA = 0xa6; // Start of 4-byte mantissa (a6, a7, a8, a9)
    ...
    static constexpr std::uint8_t ZP_AS_STRNG1 = 0xab; // Note: AS_SGNCPR and AS_STRNG1 share 0xAB in some contexts?
...
std::uint8_t AS_GOWARM = 0;      // $00 opcode byte
    ...
    std::uint16_t AS_DATLIN = 0;     // $7b/$7c line number of current AS_DATA statement
