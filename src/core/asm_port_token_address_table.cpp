...
static constexpr AS_TOKEN_ADDRESS_TABLE_fn kTokenAddressTable[] = {
    ...
    AS_POP,               // [49] $B1...177...AS_RETURN (same as AS_POP)
...
AS_TOKEN_ADDRESS_TABLE_ptr(std::size_t index) {
  return ApplesoftDualPointer<const AS_TOKEN_ADDRESS_TABLE_fn>::native(
      &kTokenAddressTable[index]);
}

AS_TOKEN_ADDRESS_TABLE_fn AS_TOKEN_ADDRESS_TABLE(
    ApplesoftDualPointer<const AS_TOKEN_ADDRESS_TABLE_fn> handler_ptr) {
  return handler_ptr.reference();
}

AS_TOKEN_ADDRESS_TABLE_fn AS_TOKEN_ADDRESS_TABLE(std::size_t index) {
  return AS_TOKEN_ADDRESS_TABLE(AS_TOKEN_ADDRESS_TABLE_ptr(index));
}
