...
void write_AS_FAC_pointer(std::uint16_t value) {
    variables().writeWord(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 1u), value);
}
...
void clear_AS_FNCNAM_hi() {
    variables().writeByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FNCNAM + 1u), 0u);
}
...
std::uint16_t read_AS_FAC_descriptor_address() {
    return variables_const().readWord(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 3u));
}

void write_AS_FAC_descriptor_address(std::uint16_t value) {
    variables().writeWord(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 3u), value);
}
...
bool AS_SUBSTRING_SETUP(SubstringSetupResult& out) {
    ...
    // Name normalization: none (assembler label AS_SUBSTRING_SETUP kept verbatim).
    AS_CHKCLS();
...
std::uint16_t AS_GETSPA(std::uint8_t length) {
    ...
    while (true) {
        // Pointer candidate lifted: AS_FRETOP/AS_FRETOP+1 and AS_STREND/AS_STREND+1 are unified words.
        const std::uint16_t startAddress = static_cast<std::uint16_t>(read_AS_FRETOP() - length);
...
// Source: SourceMaterial/Apple-II-Source-slim/src/system/applesoft/applesoft.o65.lst
// AS_Labels: AS_FIND_HIGHEST_STRING (inclusive) .. AS_CHECK_SIMPLE_VARIABLE (exclusive)
// Name normalization: none (assembler label AS_FIND_HIGHEST_STRING kept verbatim).
void AS_FIND_HIGHEST_STRING() {
    clear_AS_FNCNAM_hi();
...
// Source: SourceMaterial/Apple-II-Source-slim/src/system/applesoft/applesoft.o65.lst
// AS_Labels: AS_CHECK_SIMPLE_VARIABLE (inclusive) .. AS_CHECK_VARIABLE (exclusive)
// Name normalization: none (assembler label AS_CHECK_SIMPLE_VARIABLE kept verbatim).
void AS_CHECK_SIMPLE_VARIABLE() {
...
void AS_CHECK_BUMP() {
    write_AS_INDEX(static_cast<std::uint16_t>(read_AS_INDEX() + read_AS_DSCLEN()));
    AS_CHECK_EXIT();
}
...
// Name normalization: none (assembler label AS_MOVE_HIGHEST_STRING_TO_TOP kept verbatim).
void AS_MOVE_HIGHEST_STRING_TO_TOP() {
    ...
    const std::uint8_t lengthSlotOffset = static_cast<std::uint8_t>((read_AS_LENGTH() & 0x04u) >> 1u);
    ...
    const std::uint16_t highSource = static_cast<std::uint16_t>(sourceAddress + stringAS_Length);
    write_AS_HIGHTR(highSource);

    // AS_HIGHDS/AS_HIGHDS+1 aliases AS_ARYPNT in this memory layout.
    const std::uint16_t destination = static_cast<std::uint16_t>(read_AS_FRETOP() - stringAS_Length);
...
static void AS_CAT_impl() {
    ...
    const std::uint16_t totalAS_Length = static_cast<std::uint16_t>(first.read() + second.read());
...
void AS_MOVSTR(std::uint8_t x, std::uint8_t y, std::uint8_t length) {
    // Pointer candidate lifted: AS_INDEX low/high is one conceptual source pointer.
...
void AS_MOVSTR_1(std::uint8_t length) {
    ...
    // Carry-chain AS_FRESPC/AS_FRESPC+1 update lifted to unified pointer arithmetic.
...
void AS_PUTNEW() {
    ...
    const std::uint8_t maxTempDescriptorAddress = static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_TEMPST + 9u);
...
void AS_PUTEMP(std::uint8_t tempDescriptorAddress) {
    ...
    tempDescriptor.write(variables_const().readByte(ApplesoftVariables::ZP_AS_FAC));
    tempDescriptor.write(variables_const().readByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 1u)), 1u);
    tempDescriptor.write(variables_const().readByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 2u)), 2u);

    // AS_FAC+3/AS_FAC+4 is one logical pointer to the temp descriptor.
    write_AS_FAC_descriptor_address(static_cast<std::uint16_t>(tempDescriptorAddress));
...
//
// Free the AS_FAC string if temporary, clear AS_VALTYP to numeric, return length.
// After return AS_INDEX points at the string data.
...
void AS_GARBAG() {
    // Collect from top down: initialize AS_FRETOP from AS_MEMSIZ, then fall through.
...
bool AS_FRETMS(std::uint16_t descriptorAddress) {
    ...
    const std::uint8_t lastpt = variables_const().readByte(ApplesoftVariables::ZP_AS_LASTPT);
    const std::uint8_t lastptHi = variables_const().readByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_LASTPT + 1u));
...
//
// CHR$() built-in: parse the argument byte via AS_CONINT, allocate a 1-byte string
// with AS_STRSPA, store the character into the newly allocated space, then publish
// a temporary string descriptor with AS_PUTNEW.
// ROM pops two return addresses before jumping to AS_PUTNEW because AS_CHRSTR was
// reached via the AS_UNFNC dispatch JSR chain; in C++ the call frames unwind
...
void AS_CHRSTR() {
    ...
    // writes the converted value into AS_FAC[4] (the X-register proxy at ZP_AS_FAC+4);
    // we read it back from there.
    AS_CONINT();
    const std::uint8_t ch = variables_const().readByte(
        static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 4u));

    // Allocate space for the 1-byte string.  AS_STRSPA sets AS_FAC[0]=1 (length) and
    // AS_FAC+1,2 = allocated address.
    AS_STRSPA(1u);

    // sta (AS_FAC+1),Y  with Y=0: write the character to the allocated string space.
    // AS_FAC+1/AS_FAC+2 is one unified pointer (write_AS_FAC_pointer uses the same address).
    ...
    // jmp AS_PUTNEW: convert the raw AS_FAC string data into a temporary descriptor.
    AS_PUTNEW();
}
...
void AS_LEFTSTR() {
    ...
    const std::uint8_t copyAS_Length = std::min(setup.firstParameter, setup.sourceAS_Length);
...
void AS_RIGHTSTR() {
    ...
    const std::uint8_t copyAS_Length = std::min(setup.firstParameter, setup.sourceAS_Length);
    const std::uint8_t leftStart = static_cast<std::uint8_t>(setup.sourceAS_Length - copyAS_Length);
...
void AS_MIDSTR() {
    ...
    const std::uint8_t remaining = static_cast<std::uint8_t>(setup.sourceAS_Length - leftStart);
...
void AS_STRLT2(std::uint16_t address) {
    ...
    AS_STRINI(length);
    AS_MOVSTR(ApplesoftVariables::lowByte(start), ApplesoftVariables::highByte(start), length);
