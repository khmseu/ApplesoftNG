...
void AS_LINPRT() {
    PrintDecimalUnsigned(variables_const().AS_CURLIN);
}
...
// AS_FOUT: convert the floating-point value in AS_FAC to an AS_ASCII string buffer;
// on return (A, Y) point to the buffer.  Used inside the AS_PRINT2 number path.
// TODO(asm-port): port AS_FOUT.
static void AS_FOUT() {}

// AS_GTBYTC: advance AS_TXTPTR (via AS_CHRGET), evaluate a numeric expression, and
...
// TODO(asm-port): AS_FRETMS (temporary-release part) not yet implemented.
 std::uint8_t AS_FREFAC() {
    const std::uint16_t descriptorAddress =
        ApplesoftVariables::makeWord(variables_const().AS_FAC[3], variables_const().AS_FAC[4]);
    ...
    variables().AS_INDEX = ApplesoftVariables::makeWord(descriptorPtr.read(1u), descriptorPtr.read(2u));
...
void AS_STRPRT() {
    ...
    // which the source listing explicitly marks "<<< ABOVE THREE AS_LINES ARE USELESS >>>".
...
//
// This function captures the control-flow merge of AS_PR_STRING → AS_PRINT → AS_PRINT2
...
static void print_list_loop(std::uint8_t a, bool expr_cr) {
    while (true) {
        if (a == AS_TOKEN_TAB) {
            ...
            // Fall-through to AS_PRINT2 (T:dad7): beq AS_RTS_8 — not taken since a != 0
...
void AS_PR_COMMA() {
    ...
    // AS_PR_NEXT_CHAR (jsr AS_CHRGET / jmp AS_PRINT2) is handled by the AS_PRINT2 loop caller.
}
...
// This range also contains AS_PR_NEXT_CHAR (T:db2f) and AS_DOSPC/AS_NXSPC (T:db35).
// AS_PR_NEXT_CHAR (jsr AS_CHRGET / jmp AS_PRINT2) is the loop-back mechanism within
// the AS_PRINT2 caller; it is handled there rather than here.
void AS_PR_TAB_OR_SPC(bool is_tab) {
    ...
    // Fall through to AS_PR_NEXT_CHAR (caller handles jsr AS_CHRGET / jmp AS_PRINT2).
}
