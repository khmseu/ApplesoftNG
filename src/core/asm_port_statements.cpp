...
void SetTextPointerToInputBufferMinus1() {
    // TODO(asm-port): compute the actual AS_INPUT_BUFFER-1 address in the runtime model.
...
std::optional<TokenMatch> MatchToken(std::uint8_t index) {
    ...
    for (std::size_t i = 0; i < kTokenCount; ++i) {
        ...
        for (char expected : token) {
            if (read_AS_INPUT_BUFFER(current) != static_cast<std::uint8_t>(expected)) {
                matched = false;
                break;
            }
            ++current;
...
void AS_LET() {
    ...
    const std::uint8_t savedValTypPlus1 = ReadZeroPageByte(static_cast<std::uint8_t>(kAS_VALTYP + 1u));
...
void AS_LET2(std::uint8_t savedValTypPlus1) {
    ...
    constexpr std::uint8_t kAS_FORPNT = ApplesoftVariables::ZP_AS_FORPNT;
    constexpr std::uint8_t kAS_FAC_PLUS_3 = static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + 3u);
...
void AS_PUTSTR() {
    ...
    const std::uint8_t fretopHigh = ReadZeroPageByte(static_cast<std::uint8_t>(kAS_FRETOP + 1u));
...
void DeleteExistingAS_Line() {
    // Source: SourceMaterial/Apple-II-Source-slim/src/system/applesoft/applesoft.o65.lst
    // AS_Labels: AS_NUMBERED_LINE delete block (inclusive) .. AS_PUT_NEW_LINE (exclusive)
    // Name normalization: C++ helper; corresponds to the inline delete block inside
    // AS_NUMBERED_LINE (T:0471–T:04b5) in the assembler listing.
    ...
    const std::uint16_t lineSize = static_cast<std::uint16_t>(nextAS_Line - lowtr);
    const std::uint16_t vartab = ReadZeroPageWord(kAS_VARTAB);

    const std::uint16_t moveCount = static_cast<std::uint16_t>(vartab - nextAS_Line);
    for (std::uint16_t i = 0; i < moveCount; ++i) {
        variables().writeByte(
            static_cast<std::uint16_t>(lowtr + i),
            variables_const().readByte(static_cast<std::uint16_t>(nextAS_Line + i)));
    }
...
void InsertNewAS_Line() {
    ...
    // Name normalization: C++ helper; corresponds to AS_PUT_NEW_LINE (T:04b5–T:04f2).
    ...
    if (variables_const().readByte(ApplesoftVariables::ADDR_AS_INPUT_BUFFER) == 0u) {
        return;
    }
    ...
    while (variables_const().readByte(static_cast<std::uint16_t>(kTokenBuf + tokenAS_Len)) != 0u) {
        ++tokenAS_Len;
    }
...
std::uint16_t AdvanceToNextAS_Line(std::uint16_t current) {
    // The original AS_FIX_LINKS routine scans from the current line until it finds the
...
bool AS_FNDLIN() {
    ...
    // Assembler falls through from AS_FNDLIN directly into AS_FL1 with A=AS_TXTTAB, X=AS_TXTTAB+1.
...
bool AS_FL1(std::uint16_t startAddress) {
    ...
    const std::uint8_t targetHi = ReadZeroPageByte(static_cast<std::uint8_t>(kAS_LINNUM + 1u));
    ...
    while (true) {
        ...
        if (targetHi == lineHi) {
            const std::uint8_t lineAS_Lo = variables_const().readByte(static_cast<std::uint16_t>(current + 2u));
...
void AS_DEL() {
    ...
    if (linnumAS_Lo == 0xffu) {
        ...
        const std::uint8_t linnumHi = ReadZeroPageByte(static_cast<std::uint8_t>(kAS_LINNUM + 1u));
        WriteZeroPageByte(static_cast<std::uint8_t>(kAS_LINNUM + 1u), static_cast<std::uint8_t>(linnumHi + 1u));
    } else {
...
void AS_SAVE() {
    ...
    const std::uint16_t programAS_Length = static_cast<std::uint16_t>(programEnd - textTable);
...
void AS_LOAD() {
    ...
    WriteZeroPageWord(kAS_VARTAB, static_cast<std::uint16_t>(textTable + programAS_Length));
