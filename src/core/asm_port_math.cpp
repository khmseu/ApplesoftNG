...
void AS_FLOAT() {
    AS_FLOAT(static_cast<std::int8_t>(gFloatInput));
}
...
void AS_FADDT() {
    ...
    std::uint8_t fac_ext = ReadZeroPageByte(ApplesoftVariables::ZP_AS_FAC_EXTENSION);
...
void AS_FADD_2(std::uint8_t exponent) {
    ...
    if (diff == 0) {
        ...
        WriteZeroPageByte(ApplesoftVariables::ZP_AS_FAC_SIGN, ReadZeroPageByte(ApplesoftVariables::ZP_AS_ARG_SIGN));
...
void AS_MUL10() {
    ...
    WriteZeroPageByte(ApplesoftVariables::ZP_AS_FAC, exp + 1); // AS_FAC*5 * 2 = AS_FAC*10
}
 ...
 */
void AS_NEGATE_FAC() {
    if (ReadZeroPageByte(ApplesoftVariables::ZP_AS_FAC) == 0) return;
...
void AS_COPY_FAC_TO_ARG_ROUNDED() {
    ...
    for (int i = 0; i < 6; ++i) {
        std::uint8_t val = ReadZeroPageByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + i));
        WriteZeroPageByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_ARG + i), val);
    }
...
void AS_INCREMENT_FAC_MANTISSA() {
    for (int i = 4; i >= 1; --i) {
        std::uint8_t addr = static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + i);
...
void AS_FLOAT(std::int8_t value) {
    WriteZeroPageByte(ApplesoftVariables::ZP_AS_FAC + 1, static_cast<std::uint8_t>(value));
...
void AS_FIN() {
    ...
    // Note: AS_CHRGET already populated AS_CHARAC and set C flag based on if it's a digit.
    // In our C++ port, we'll use AS_CHRGET() to get the current state.
...
AS_FIN_10:
    // AS_DPFLG is $9B. Use BIT/ROR logic: first time sets $80, second time sets $C0
    {
...
void AS_ADDACC_WITH_DIGIT(std::uint8_t digit) {
    ...
    // AS_SGNCPR is $AB
    WriteZeroPageByte(0xAB, ReadZeroPageByte(ApplesoftVariables::ZP_AS_ARG_SIGN) ^ ReadZeroPageByte(ApplesoftVariables::ZP_AS_FAC_SIGN));
    AS_FADDT();
}
 ...
 */
void AS_ADD_EXPONENTS() {
    std::uint16_t sum = (std::uint16_t)ReadZeroPageByte(ApplesoftVariables::ZP_AS_FAC) + ReadZeroPageByte(ApplesoftVariables::ZP_AS_ARG);
    if (sum > 0xFF) {
        AS_ERROR(0x45); // AS_OVERFLOW
        return;
    }
    WriteZeroPageByte(ApplesoftVariables::ZP_AS_FAC, static_cast<std::uint8_t>(sum));
}
...
void AS_COPY_ARG_TO_FAC() {
    ...
    for (int i = 0; i < 5; ++i) {
        std::uint8_t val = ReadZeroPageByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_ARG + i));
        WriteZeroPageByte(static_cast<std::uint8_t>(ApplesoftVariables::ZP_AS_FAC + i), val);
    }
    WriteZeroPageByte(ApplesoftVariables::ZP_AS_FAC_EXTENSION, 0);
}
