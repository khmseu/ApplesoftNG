...
constexpr std::uint16_t kAS_INPUT_BUFFER_MINUS_1 =
    ApplesoftVariables::ADDR_AS_INPUT_BUFFER_MINUS_1;
constexpr std::uint16_t kAS_INPUT_BUFFER =
    ApplesoftVariables::ADDR_AS_INPUT_BUFFER;
constexpr std::uint16_t kAS_INPUT_BUFFER_PLUS_1 =
    ApplesoftVariables::ADDR_AS_INPUT_BUFFER + 1u;
  ...
  while (true) {
    ...
    if (next_program_char == 0u) {
      ...
      if ((variables_const().AS_INPUTFLG & 0x80u) != 0u) {
        ...
        if (variables_const().pointer(variables_const().AS_INPTR).read() !=
            0u) {
